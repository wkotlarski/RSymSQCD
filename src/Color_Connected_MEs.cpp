...
double Color_Connected_MEs::get_ME2_value(const std::vector<Vec4D<double>> & v) const {
   auto [k12, k23, k24, k13, k14, k34] = std::tuple(
           v[0]*v[1], v[1]*v[2], v[1]*v[3], v[0]*v[2], v[0]*v[3], v[2]*v[3]
   );
   return uu_suLsuR(k12, k23, k24, k13, k14, k34);
}
...
double Color_Connected_MEs::ColorMatrix(std::string const& col_str1, std::string const& col_str2 ) const {
   ...
   // @todo need to distinguish between CF & CA
   return temp/CF;
}

double Color_Connected_MEs::uu_suLsuR(double k12, double k23, double k24, double k13, double k14, double k34) const {
