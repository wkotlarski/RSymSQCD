double MRSSM::matrixTree_ubarg_suRdaggerglbar(const double alphas, const double S, const double T, const int Dminus4SeriesCoeff) const {
   ...
   if (Dminus4SeriesCoeff == 0) {
      res = (-4*(alphas*alphas)*(pi*pi)*(4*(S*S) + S*(MassSq2 - U) + 4*pow<2>(-MassSq2 + U))*(5*pow<3>(MassSq2) - pow<2>(MassSq2)*(7*S + 6*T + 9*U) - U*(2*T*(S + T) + (S + 2*T)*U + U*U) + MassSq2*(2*pow<2>(S + T) + 6*S*U + 8*T*U + 5*(U*U))))/(9.*S*pow<2>(MassGlu2 - T)*pow<2>(-MassSq2 + U));
   }
