...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_0(w[4], w[10], w[6], pars->GC_12, amp[3]); 
  VVV1_0(w[0], w[8], w[6], pars->GC_10, amp[4]); 
  FFV1_0(w[4], w[1], w[11], pars->GC_12, amp[5]); 
  FFV1_0(w[4], w[1], w[12], pars->GC_12, amp[6]); 

}
double CPPProcess::matrix_1_gux_sig8psig8pux() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 7; 
