	...
	!PD 
     !select case (renormalisation)
      ...
      case (0)
         ! no renormalisation
      case (1)
         ! fully renormalized !PD
         !print*, amp(2)/amp(1),amp(1),amp(3)/amp(1)
         call renormalize(scale2,vecs,amp,my_ok) !PD
         !print*, amp(2)/amp(1),amp(1),amp(3)/amp(1)
      case default
         ! not implemented
         print*, "In [% process_name asprefix=\_ %]matrix:"
...
@end @if generate_lo_diagrams %]
   ...
   subroutine renormalize(scale2,vecs,amp,ok)
     ...
     m2 = mdlmsur
     ! sql sqr prod
     t = m1*m1-2*(vecs(1,1)*vecs(3,1)-vecs(1,2)*vecs(3,2)-vecs(1,3)*vecs(3,3)-vecs(1,4)*vecs(3,4))
     ...
     !! sql sqr diff flavor if flavor from 1 to 3 and 2 to 4 is the same else T->U
     tempmg = 2 * mdlmd3**2/(T-mdlmd3**2)
     !! q qbar sq sqbar same flavor else 0
     ! tempmg = (2*mdlmd3*mdlmd3*S*(mdlmd3**2 + 3*S - T))/ &
     !     &((-mdlmd3**2 + T)*(6*mdlmd3**4 + 3*S**2 + 2*mdlmd3**2*(S - 6*T) - 2*S*T + 6*T**2))
     ! print*, tempmg
     ! wave function renormalisation:
     amp(3) = amp(3) + 2.0_ki * (dzq(-1._ki,m1) + dzsq(-1._ki,m1) + dmgl(-1._ki) * tempmg) * amp(1)
     amp(2) = amp(2) + 2.0_ki * (dzq(0._ki,m1 ) + dzsq(0._ki,m1) + dmgl(0._ki ) * tempmg) * amp(1)
     amp(2) = amp(2) + 4.0_ki * dzg(0._ki,scale2) * amp(1)
     !print*, 2.0_ki * (dzq(0._ki ) + dzsq(0._ki,m1 ) + dmgl(0._ki ) * tempmg)
     !print*,  4.0_ki * dzg(0._ki,scale2) 
     !print*,  4.0_ki * dzg(0._ki, -1._ki) 
     !call ltexi
  ...
  function testreno(epsorder) result(dz)
   ...
   print *, B0(0.d0,mdlmd3,mdlmsul)
   print *, B0(0.d0,mdlmd3**2,mdlmsul**2)
    dz = B0(0.d0,mdlmd3**2,mdlmsul**2)
  end function testreno

! equal squark mass limit else left and right ct have different msq dependency
! take out factor alpha/2pi^2
  function dzq(epsorder,msq) result(dz)
    implicit none
    real(ki), intent(in) :: epsorder,msq
    real(ki) :: dz
    dz=0.0_ki

    call setlambda(epsorder)
    ! if (epsorder.eq.0.d0) then
    !    dz =  -0.5_ki * CF ! seems like the dr to cdr ct
    ! else
    !    dz = 0.0_ki
    ! endif
    dz = dz + CF * real(B1(0._ki, mdlmd3**2, msq**2))
  end function dzq

! should be different for all flavours could do by flag
! no stop!!!
  function dzsq(epsorder,msq) result(dz)
    implicit none
    ...
    dz = dz + 0.5_ki * CF * (3._ki * real(B0i(bb0, msq**2, 0._ki, msq**2)) + &
         & 2._ki * (2._ki * real(B0i(bb1, msq**2, 0._ki, mdlmd3**2)) + &
         &    real(B0i(bb1, msq**2, 0._ki, msq**2)) + &
         &    2._ki * msq**2 * real(B0i(dbb0, msq**2, 0._ki, msq**2)) -& 
         &    2._ki * mdlmd3**2 * real(B0i(dbb0, msq**2, mdlmos**2, msq**2)) +& 
         &    2._ki * msq**2 * real(B0i(dbb1, msq**2, 0._ki, mdlmd3**2)) +& 
         &   msq**2 * real(B0i(dbb1, msq**2, 0._ki, msq**2))))
  end function dzsq

  function dzgluon(epsorder) result(dz)
    implicit none
    real(ki), intent(in) :: epsorder
    real(ki) :: dz
    dz=0.0_ki
    ...
    dz = dz + CA *( &
       & 2*real(B0i(bb1,0,mdlmd3**2,mdlmd3**2)) + 2*mdlmd3**2*real(B0i(dbb0,0,mdlmd3**2,mdlmd3**2)) - &
       & 2*mdlmd3**2 * real(B0i(dbb0,0,mdlmd3**2,mdlmd3**2)) - 4*real(B0i(dbb00,0,mdlmd3**2,mdlmd3**2)) + & 
       & real(B0i(dbb00,0,mdlmos**2,mdlmos**2)) + real(B0i(dbb00,0,mdlmop**2,mdlmop**2)) + &
       & real(B0i(bb1,0,mdlmt**2,mdlmt**2))/3  - 2*real(B0i(dbb00,0,mdlmt**2,mdlmt**2))/3. + &
       & real(B0i(dbb00,0,mdlmsdl**2,mdlmsdl**2))/3. + real(B0i(dbb00,0,mdlmsul**2,mdlmsul**2))/3 + &
       & real(B0i(dbb00,0,mdlmsdr**2,mdlmsdr**2))/3. + real(B0i(dbb00,0,mdlmsur**2,mdlmsur**2))/3 +&
       & real(B0i(dbb00,0,mdlmssl**2,mdlmssl**2))/3. + real(B0i(dbb00,0,mdlmscl**2,mdlmscl**2))/3 +&
       & real(B0i(dbb00,0,mdlmssr**2,mdlmssr**2))/3. + real(B0i(dbb00,0,mdlmscr**2,mdlmscr**2))/3 +&
       & real(B0i(dbb00,0,mdlmsbl**2,mdlmsbl**2))/3. + real(B0i(dbb00,0,mdlmstl**2,mdlmstl**2))/3 +&
       & real(B0i(dbb00,0,mdlmsbr**2,mdlmsbr**2))/3. + real(B0i(dbb00,0,mdlmstr**2,mdlmstr**2))/3 )

  end function dzgluon
 ! finite CA/2 missing?
  function dzxglL(epsorder) result(dz)
    implicit none
    real(ki), intent(in) :: epsorder
    real(ki) :: dz
    ...
    call setlambda(epsorder)
    dz = dz + CA * real( &
         & -B0i(bb0,mdlmd3**2,0,mdlmd3**2) - B0i(bb1,mdlmd3**2,0,mdlmd3**2) + &
         &  B0i(bb1,mdlmd3**2,mdlmd3**2,mdlmos**2)/2. + B0i(bb1,mdlmd3**2,mdlmd3**2,mdlmop**2)/2. - &
         &  2*mdlmd3**2*B0i(dbb0,mdlmd3**2,0.0_ki,mdlmd3**2) - 2*mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmd3**2) + &
         &  mdlmd3**2*B0i(dbb1,mdlmd3**2,mdlmd3**2,mdlmos**2) &
         & + mdlmd3**2*B0i(dbb1,mdlmd3**2,mdlmd3**2,mdlmop**2) + &
         & 4*mdlmd3*B0i(dbb0,mdlmd3**2,0.0_ki,mdlmd3**2)*mdlmd3 - &
         & mdlmd3*B0i(dbb0,mdlmd3**2,mdlmd3**2,mdlmos**2)*mdlmd3 + &
         & mdlmd3*B0i(dbb0,mdlmd3**2,mdlmd3**2,mdlmop**2)*mdlmd3 + &
         & (B0i(bb1,mdlmd3**2,0.0_ki,mdlmsdl**2))/6. + (B0i(bb1,mdlmd3**2,0.0_ki,mdlmsdr**2))/6. + &
         & (B0i(bb1,mdlmd3**2,0.0_ki,mdlmssl**2))/6. + (B0i(bb1,mdlmd3**2,0.0_ki,mdlmssr**2))/6. + &
         & (B0i(bb1,mdlmd3**2,0.0_ki,mdlmsbl**2))/6. + (B0i(bb1,mdlmd3**2,0.0_ki,mdlmsbr**2))/6. + &
         & (B0i(bb1,mdlmd3**2,0.0_ki,mdlmsul**2))/6. + (B0i(bb1,mdlmd3**2,0.0_ki,mdlmsur**2))/6. + &
         & (B0i(bb1,mdlmd3**2,0.0_ki,mdlmscl**2))/6. + (B0i(bb1,mdlmd3**2,0.0_ki,mdlmscr**2))/6. + &
         & (B0i(bb1,mdlmd3**2,mdlmt**2,mdlmstl**2))/6. + (B0i(bb1,mdlmd3**2,mdlmt**2,mdlmstr**2))/6. + &
         & (mdlmd3**2*B0i(dbb1,mdlmd3**2,0.0_ki,mdlmsdl**2))/6. & 
         & + (mdlmd3**2*B0i(dbb1,mdlmd3**2,0.0_ki,mdlmsdr**2))/6. + &
         & (mdlmd3**2*B0i(dbb1,mdlmd3**2,0.0_ki,mdlmssl**2))/6. & 
         & + (mdlmd3**2*B0i(dbb1,mdlmd3**2,0.0_ki,mdlmssr**2))/6. + &
         & (mdlmd3**2*B0i(dbb1,mdlmd3**2,0.0_ki,mdlmsbl**2))/6. &
         & + (mdlmd3**2*B0i(dbb1,mdlmd3**2,0.0_ki,mdlmsbr**2))/6. + &
         & (mdlmd3**2*B0i(dbb1,mdlmd3**2,0.0_ki,mdlmsul**2))/6. + & 
         & (mdlmd3**2*B0i(dbb1,mdlmd3**2,0.0_ki,mdlmsul**2))/6. + &
         & (mdlmd3**2*B0i(dbb1,mdlmd3**2,0.0_ki,mdlmscl**2))/6. + &
         & (mdlmd3**2*B0i(dbb1,mdlmd3**2,0.0_ki,mdlmscl**2))/6. + &
         & (mdlmd3**2*B0i(dbb1,mdlmd3**2,mdlmt**2,mdlmstl**2))/6. + & 
         & (mdlmd3**2*B0i(dbb1,mdlmd3**2,mdlmt**2,mdlmstl**2))/6. )
   end function dzxglL

  function dzxglR(epsorder) result(dz)
    implicit none
    real(ki), intent(in) :: epsorder
    real(ki) :: dz
    dz=0.0_ki
    ...
    call setlambda(epsorder)
    dz= dz+ CA * real( &
         &    - 6*B0i(bb0,mdlmd3**2,0,mdlmd3**2) - 6*B0i(bb1,mdlmd3**2,0,mdlmd3**2) +&
         & 3*B0i(bb1,mdlmd3**2,mdlmd3**2,mdlmop**2) + 3*B0i(bb1,mdlmd3**2,mdlmd3**2,mdlmos**2) + &
         & 12*mdlmd3**2*B0i(dbb0,mdlmd3**2,0,mdlmd3**2) + 6*mdlmd3**2*B0i(dbb0,mdlmd3**2,mdlmd3**2,mdlmop**2) - &
         & 6*mdlmd3**2*B0i(dbb0,mdlmd3**2,mdlmd3**2,mdlmos**2) - 12*mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmd3**2) + & 
         & mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmsbl**2) + mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmsbr**2) + & 
         & mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmscl**2) + mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmscr**2) + & 
         & mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmsdl**2) + mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmsdr**2) + & 
         & mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmssl**2) + mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmssr**2) + & 
         & mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmsul**2) + mdlmd3**2*B0i(dbb1,mdlmd3**2,0,mdlmsur**2) + & 
         &  6*mdlmd3**2*B0i(dbb1,mdlmd3**2,mdlmd3**2,mdlmop**2) + &
         &  6*mdlmd3**2*B0i(dbb1,mdlmd3**2,mdlmd3**2,mdlmos**2) + & 
         & mdlmd3**2*B0i(dbb1,mdlmd3**2,mdlmt**2,mdlmstl**2) + & 
         &  mdlmd3**2*B0i(dbb1,mdlmd3**2,mdlmt**2,mdlmstr**2))/6.0_ki
  end function dzxglR

  !factor of g3 multiplied out so that no coupling when alpha/2pi divided out
  !dg = dalpha/2
  function dzg(epsorder,mu2) result(dz)
    implicit none
    real(ki), intent(in) :: epsorder, mu2
    real(ki) :: dz
    if (epsorder.eq.0.d0) then
       dz = 0.25_ki  !MS to DR
       if (mu2.gt.1._ki)  then
          dz = dz - (Log(mdlmd3**2/mu2) +  Log(mdlmos**2/mu2) / 8._ki +  Log(mdlmop**2/mu2) / 8._ki &
               &  + ( 0.5_ki * Log(mdlmsur**2/mu2) + 0.5_ki * Log(mdlmsul**2/mu2) & 
               &  + 0.5_ki * Log(mdlmsdr**2/mu2) + 0.5_ki * Log(mdlmsdl**2/mu2) & 
               &  + 0.5_ki * Log(mdlmscr**2/mu2) + 0.5_ki * Log(mdlmscl**2/mu2) & 
               &  + 0.5_ki * Log(mdlmssr**2/mu2) + 0.5_ki * Log(mdlmssl**2/mu2) & 
               &  + 0.5_ki * Log(mdlmsbr**2/mu2) + 0.5_ki * Log(mdlmsbl**2/mu2) & 
               &  + 0.5_ki * Log(mdlmstr**2/mu2) + 0.5_ki * Log(mdlmstl**2/mu2))/12._ki & 
               &  + Log(mdlmt**2/mu2) / 6._ki)
       end if
    else
       dz=0.0_ki
    end if
  ...
  function dmgl(epsorder) result(dz)
    implicit none
    real(ki), intent(in) :: epsorder
    real(ki) :: dz
    ...
    ! if (epsorder.eq.0.d0) the    !    dz = 0.5_ki * CA 
    ! else 
    dz=0.0_ki
    !    end if
    dz = dz + 0.25_ki * ( - 12 * real(B0i(bb0, mdlmd3**2, 0.d0, mdlmd3**2)) + &
         &  6._ki * real(B0i(bb0, mdlmd3**2, mdlmd3**2, mdlmos**2)) - &
         &  6._ki * real(B0i(bb0, mdlmd3**2, mdlmd3**2, mdlmop**2)) + &
         &  12._ki * real(B0i(bb1, mdlmd3**2, 0.d0, mdlmd3**2)) - &
         &  real(B0i(bb1, mdlmd3**2, 0.d0, mdlmsdl**2)) - &
         &  real(B0i(bb1, mdlmd3**2, 0.d0, mdlmssl**2)) - &
         &  real(B0i(bb1, mdlmd3**2, 0.d0, mdlmsbl**2)) - &
         &  real(B0i(bb1, mdlmd3**2, 0.d0, mdlmsdr**2)) - &
         &  real(B0i(bb1, mdlmd3**2, 0.d0, mdlmssr**2)) - &
         &  real(B0i(bb1, mdlmd3**2, 0.d0, mdlmsbr**2)) - &
         &  real(B0i(bb1, mdlmd3**2, 0.d0, mdlmsul**2)) - &
         &  real(B0i(bb1, mdlmd3**2, 0.d0, mdlmsur**2)) - &
         &  6._ki * real(B0i(bb1, mdlmd3**2, mdlmd3**2, mdlmos**2)) - &
         &  6._ki * real(B0i(bb1, mdlmd3**2, mdlmd3**2, mdlmop**2)) - &
         &  real(B0i(bb1, mdlmd3**2, 0.d0, mdlmscl**2)) - &
         &  real(B0i(bb1, mdlmd3**2, 0.d0, mdlmscr**2)) - &
         &  real(B0i(bb1, mdlmd3**2, mdlmt**2, mdlmstl**2)) - &
         &  real(B0i(bb1, mdlmd3**2, mdlmt**2, mdlmstr**2)))
    
   ! dz =dz *mdlmd3
  end function dmgl

  function dmsq(epsorder, msq) result(dz)
    implicit none
    real(ki), intent(in) :: epsorder, msq
    real(ki) :: dz
    ...
    dz=0.0_ki
    !    end if
    dz = dz + (0.5_ki*CF *(-4*Real(A0(mdlmd3**2)) + Real(A0(msq**2)) - &
